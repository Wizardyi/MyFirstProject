...
public:
	...
	//可能实现以下函数
	//设置报头数据
	virtual	void pkg_header( uint32_t userid,uint16_t cmdid ) {
	...
	//可能实现以下函数
	//得到ret
	virtual	int get_result( char * buf, uint32_t buf_len ){
	...
	//可能实现以下函数
	//设置报头部分的长度
	virtual	int set_pkg_header_total_len( uint32_t len ){
		*((uint32_t *)this->ba_in_.get_buf())=len;
	}



	void set_pkg_header_len(uint32_t head_len ){
		this->proto_header_len_=head_len;
	}


	int32_t get_proto_header_len( ){
		return this->proto_header_len_;
	}
	...
	inline int deal_proto_cmd(uint32_t userid,uint16_t cmdid,Cmessage *c_in, Cmessage *c_out  )
	{
		...
		this->set_pkg_header_total_len(this->ba_in_.get_postion() );
		int ret;
