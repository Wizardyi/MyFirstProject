...
public:
	...
	byte_array_t(bool a_is_bigendian ){
		this->m_is_read_only=true;
		this->m_buf=NULL;
		this->is_bigendian=a_is_bigendian;
	}
	...
	byte_array_t(uint32_t malloc_size=32, bool a_is_bigendian=false  ){//写入模式
...
//设置变长字段
#define PROTO_SET_VARCHAR( obj, field_name, buf, buf_len  )				\
	do {																\
		(obj)._## field_name ##_len =buf_len<sizeof((obj).field_name )?	\
											 buf_len: sizeof((obj).field_name ) ; \
			memcpy( (obj).field_name , buf, (obj)._## field_name ##_len   ); \
	}while(0)															\
