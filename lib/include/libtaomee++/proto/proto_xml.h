 ...
 *
 * Last-Updated: 2013-06-25 17:57:26
 *           By: jim
 *     Update #: 132
 *
 * 
...
template <typename  Type >
inline void str2array (const char * src, Type *  p_arr, uint32_t size  ){
	std::string str(src);
	std::replace_if(str.begin(),str.end(), str2vector_check_set_space ,' ');
	std::stringstream ss(str );
	memset(p_arr,0,sizeof(Type)*size );
	double value; 
	uint32_t i=0;
	while( ss>>value ){
		if (i>=size){//已经够了
...
inline  void debug_log_cmsg(  Cmessage *p_msg ){                                           
    byte_array_t ba;
    ba.init_postion();
    p_msg->write_to_buf(ba);
    char outbuf[13000];
    bin2hex(outbuf,(char*)ba.get_buf() ,ba.get_postion(),2000);
    DEBUG_LOG("CMSG: %s %s ", p_msg->_get_type_id(), outbuf   );
}
