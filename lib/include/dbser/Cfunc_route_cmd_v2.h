...
public:
	...
	inline void set_is_log_hex_pkg(bool value )	{
 		 this->is_log_hex_pkg=value;
	}
	...
	*/	
			...
			if((p_cmd_item =this->get_cmd_item(item.cmdid ))!=NULL){
				...
				if (this->ret==SUCC){//需要构造返回报文
					...
					if (!p_cmd_item->msg_out->write_to_buf(this->ba_out)) {
						KDEBUG_LOG(userid,"p_cmd_item->msg_out->write_to_buf :SYS_ERR" );
						return SYS_ERR;
					}
				}else{//返回错误
				...
				KDEBUG_LOG(userid,"cmd no define  cmdid[%04X]",cmdid );
		...
		{
			KDEBUG_LOG(userid,"set_std_return:SYS_ERR: %u",
	...
	{
		uint16_t cmdid=((PROTO_HEADER*)recvbuf)->cmd_id;
		uint32_t userid=((PROTO_HEADER*)recvbuf)->id;
		if (!this->is_log_hex_pkg){
			KDEBUG_LOG ( userid ,"I:%04X", cmdid );
		}else{
	        char outbuf[13000];
	        bin2hex(outbuf,(char*)recvbuf,rcvlen,500);
			KDEBUG_LOG ( userid ,"I:%04X:[%s]", cmdid,outbuf );
		}
		...
		if(this->collect_cmd_set.find(cmdid)!=this->collect_cmd_set.end()){
			...
			ret=deal_one_cmd(recvbuf, rcvlen, sendbuf, sndlen );
			if (ret==SUCC && this->is_log_hex_pkg){
		        char outbuf[13000];
		        bin2hex(outbuf,(char*)*sendbuf,*sndlen ,500);
				KDEBUG_LOG ( userid ,"O:%04X:[%s]", cmdid,outbuf );
			}else{
				KDEBUG_LOG ( userid ,"O:%04X:%d", cmdid,ret );
			}
			return ret;
