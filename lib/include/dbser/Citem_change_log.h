...
protected:
	...
	int insert(uint32_t logtime, uint32_t opt_type,
			   uint32_t server_id  ,
			   uint32_t userid,  uint32_t role_id,
			uint32_t  itemid, uint32_t count ) ;

	int create_table( uint32_t userid,uint32_t logtime);
	int create_table_ex( uint32_t userid,uint32_t logtime);
...
public:
	...
	// is_vip_opt:是不是vip 渠道
	// count :可以为+/- 
	int add(uint32_t logtime, bool is_vip_opt,  uint32_t server_id  ,
			uint32_t userid,  uint32_t role_id,
			uint32_t  itemid, int count ) ;
