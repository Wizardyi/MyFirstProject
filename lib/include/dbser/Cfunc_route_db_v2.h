...
protected: 
	...
	bool is_can_noti_proxy_use_other_server_on_mysql_err;
	//db 连接
	mysql_interface *db;
	//在线同步
...
public:
	...
	virtual int deal_func(uint16_t cmdid, userid_t userid, Cmessage * c_in, Cmessage * c_out ,P_DEALFUN_T p_func ,bool is_commit )
	{
		...
		//调用相关DB处理函数
		this->db->set_last_mysql_errno(0);
		this->ret=9999;
		this->ret=(((Croute_func*)this)->*(p_func))(
				userid,c_in ,c_out);	
		//DEBUG_LOG("ret test=-===%d,%d,%d",this->is_can_noti_proxy_use_other_server_on_mysql_err, this->ret ,this->db->get_last_mysql_errno() );
		if (this->is_can_noti_proxy_use_other_server_on_mysql_err 
				&& this->ret==DB_ERR 
				&& (this->db->get_last_mysql_errno()==2003 ||this->db->get_last_mysql_errno()==2006 )
		   ){
			this->ret=501;//noti proxy change to other server
		}
	...
	inline void set_is_can_noti_proxy_use_other_server_on_mysql_err(bool value ){
		this->is_can_noti_proxy_use_other_server_on_mysql_err=value;
	}
