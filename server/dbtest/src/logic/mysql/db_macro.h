...
#define set_mysql_string(dst,src,n)	mysql_real_escape_string(  \
		  this->db->conn, dst, src, n)
#define STD_ROLLBACK()  {\
	DEBUG_LOG("[D] DO:STD_ROLLBACK" );\
	mysql_rollback(this->db->conn);\
}
#define STD_CLOSE_AUTOCOMMIT(nret) {\
		if (mysql_autocommit(this->db->conn, F)==DB_SUCC){\
				nret = DB_SUCC;\
		}else {\
				nret = DB_ERR;\
		}\
...
#define STD_COMMIT(nret) {\
		if (mysql_commit(this->db->conn) == DB_SUCC){\
				nret = DB_SUCC;\
		}else {\
				nret = DB_ERR;\
		}\
...
#define BIN_CPY_NEXT_FIELD_TO_MSG(item_name, msg_name)  ++_fi; \
		mysql_fetch_lengths(res); \
		item_name._##msg_name##_len=res->lengths[_fi]<sizeof(item_name.msg_name) ? res->lengths[_fi]:sizeof(item_name.msg_name);\
		memcpy(item_name.msg_name,row[_fi],item_name._##msg_name##_len );


//copy 到CMESSAGE 用于复制
#define BIN_CPY_NEXT_FIELD_TO_CMESSAGE(msg_name)  ++_fi; \
		mysql_fetch_lengths(res); \
		(msg_name).read_from_buf_ex(row[_fi], res->lengths[_fi]);
