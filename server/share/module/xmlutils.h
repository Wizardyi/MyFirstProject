...
/* @brief 检查 v 是否已经在其他地方使用了
 */
#define COMMON_DATA_CHECK(v) \
		do { \
			if (v != 0) { \
				std::set<uint32_t>::iterator it = common_data_key.find(v); \
				if (it == common_data_key.end() ) { \
					common_data_key.insert(v); \
				} \
				else { \
					ERROR_LOG("the common data :%u is being used!", v); \
				} \
			} \
		} while (0)
