...
extern "C" int  init_service(int isparent)
{
	if (!isparent) 
	{
		...
		g_server_config = new server_config_t();
...
//             DEBUG_LOG("add proxy conn timer");
...
{
	//    // online->cache
	//g_proto_processor->register_command(cacheproto::MessageType::LCCreateRole,new LCCreateRoleProcessor());

	//g_proto_processor->register_command(cacheproto::MessageType::WCGetOpenNewActivity,new WCGetOpenNewActivityProcessor());
	//g_proto_processor->register_command(cacheproto::MessageType::WCSetOpenNewActivity,new WCSetOpenNewActivityProcessor());
...
//    if (!dbproxy_name) {
//        ERROR_LOG("NO DBPROXY CONFIG FOUND IN BENCH.CONF");
//        return -1;
//    }
