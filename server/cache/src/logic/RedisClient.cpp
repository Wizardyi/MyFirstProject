...
#define BIND_ORDERPAIR(val) std::bind(&FetchOrderPair, std::placeholders::_1, val)
...
static inline int FetchOrderPair(redisReply *pReply, std::vector<std::pair<std::string, std::string> > *pmapFv)
{
    if (pReply->type == REDIS_REPLY_ARRAY)
    {
        int nRet = RC_SUCCESS;
        ...
        for (size_t i = 0; i < pReply->elements; )
        {
            ...
            if (nSubRet == RC_SUCCESS)
                pmapFv->push_back(std::make_pair(strFld, strVal));
            else
                return nSubRet;
        }
        return nRet;
...
int CRedisConnection::ConnRequest(CRedisCommand *pRedisCmd)
{
	...
	// INFO_LOG("USER TIME:%u, NOW TIME:%u", m_nUseTime, tmNow);
...
bool CRedisConnection::ConnectToRedis(
		const std::string &strHost, int nPort, int nTimeout,
		const std::string& strInstanceID, const std::string& strPasswd)
{
    if (m_pContext)
        redisFree(m_pContext);
	...
	INFO_LOG("AUTH %s:%s", strInstanceID.c_str(), strPasswd.c_str());
	std::string ok("OK");
	if (!strInstanceID.empty() && !strPasswd.empty()) {
		redisReply* reply = (redisReply*)redisCommand(
				m_pContext, "AUTH %s:%s", strInstanceID.c_str(), strPasswd.c_str());
		if (strcmp(reply->str, ok.c_str())) {
		...
		redisReply* reply = (redisReply*)redisCommand(m_pContext, "AUTH %s", strPasswd.c_str());
		if (strcmp(reply->str, ok.c_str())) {
			freeReplyObject(reply);
			return false;
		}
...
bool CRedisConnection::Reconnect()
{
    if (!m_pRedisServ->m_strHost.empty() &&
        ConnectToRedis(
			m_pRedisServ->m_strHost, m_pRedisServ->m_nPort, m_pRedisServ->m_nCliTimeout,
			m_pRedisServ->m_strInstanceID, m_pRedisServ->m_strPasswd))
        return true;

    for (auto &hostPair : m_pRedisServ->m_vecHosts)
    {
        if (ConnectToRedis(
					hostPair.first, hostPair.second, m_pRedisServ->m_nCliTimeout,
					m_pRedisServ->m_strInstanceID, m_pRedisServ->m_strPasswd))
        {
...
// m_nSerTimeout 使用参数2，表示该条连接，有2秒没有使用会进行连接重置，保证连接是可用的
// 不能小于等于1，因为2次操作的时间间隔有可能在1秒内，会导致连接频繁重置。
CRedisServer::CRedisServer(
		const std::string &strHost, int nPort, int nTimeout, int nConnNum,
		const std::string& strInstanceID, const std::string& strPasswd)
    : m_strHost(strHost), m_nPort(nPort), m_nCliTimeout(nTimeout), m_nSerTimeout(2), m_nConnNum(nConnNum), m_strInstanceID(strInstanceID), m_strPasswd(strPasswd)
{
		...
		const std::string& strInstanceID, const std::string& strPasswd)
{
    ...
    CRedisServer *pRedisServ = new CRedisServer(
			m_strHost, m_nPort, m_nTimeout, m_nConnNum, m_strInstanceID, m_strPasswd);
...
int CRedisClient::Setbit(const std::string &strKey, const std::string& strOffset, bool bVal, Pipeline ppLine)
{
    return ExecuteImpl("setbit", strKey, strOffset, ConvertToString((long)bVal), HASH_SLOT(strKey), ppLine, BIND_INT(nullptr));
}
...
int CRedisClient::Zadd(const std::string &strKey, const std::string& strScore, const std::string &strElem, long *pnVal, Pipeline ppLine)
{
    return ExecuteImpl("zadd", strKey, strScore, strElem, HASH_SLOT(strKey), ppLine, BIND_INT(pnVal));
}
...
int CRedisClient::Zrevrangewithscore(const std::string &strKey, long nStart, long nStop, std::vector<std::pair<std::string, std::string> > *pmapVal, Pipeline ppLine)
{
    return ExecuteImpl("zrevrange", strKey, ConvertToString(nStart), ConvertToString(nStop), std::string("WITHSCORES"), HASH_SLOT(strKey), ppLine, BIND_ORDERPAIR(pmapVal));
}

int CRedisClient::Zrangewithscore(const std::string &strKey, long nStart, long nStop, std::vector<std::pair<std::string, std::string> > *pmapVal, Pipeline ppLine)
{
    return ExecuteImpl("zrange", strKey, ConvertToString(nStart), ConvertToString(nStop), std::string("WITHSCORES"), HASH_SLOT(strKey), ppLine, BIND_ORDERPAIR(pmapVal));
}
...
int CRedisClient::Zrevrangebyscore(const std::string &strKey, const std::string& strMax, const std::string& strMin, std::vector<std::string> *pvecVal, Pipeline ppLine)
{
    return ExecuteImpl("zrevrangebyscore", strKey, strMax, strMin, HASH_SLOT(strKey), ppLine, BIND_VSTR(pvecVal));
}
    ...
    for (size_t i = 0; i < m_vecRedisServ.size(); ++i)
    {
            ...
            redisCmd.FetchResult(BIND_SLOT(&vecSlot)) == RC_SUCCESS)
        {
            for (auto &slotReg : vecSlot)
            {
                ...
                {
                    pSlotServ = new CRedisServer(
							slotReg.strHost, slotReg.nPort, m_nTimeout, m_nConnNum,
							m_strInstanceID, m_strPasswd);
